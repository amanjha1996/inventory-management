...
<h2><center>welcome to search menu</center></h2>
    ...
    enter the location id for search<input type="text"  name="id">
...
try
{
  ...
  ResultSet rs = st.executeQuery("select * from location where L_id="+id+"");
  if(rs.next()){
      %>
      <center><h1>SEARCH RESULT</h1>
          <table BORDER="1"> <h3><br> <tr><td>location_id:</td><td><%out.println(id);%> </td></tr></h3>
          <h3><br> <tr><td>location_Name: </td><td><%=rs.getString("L_name")%> </td></tr></h3>
          </table>
          </center><%
  }
